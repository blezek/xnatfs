...
public class XNATConnection {
   ...
   */
  /*
   * synchronized public RemoteFileHandle get ( String s, String path ) throws
   * Exception { String URL = "http://" + mHost + ":" + mPort + mPrefix + s;
   * logger.debug ( "Trying to get: " + URL ); // GetMethod get = new GetMethod
   * ( URL ); // mClient.executeMethod ( get ); return new RemoteFileHandle (
   * URL, path ); }
   * 
   * synchronized public FileHandle getFileHandle ( String s, String path )
   * throws Exception { String URL = "http://" + mHost + ":" + mPort + mPrefix +
   * s; logger.debug ( "Trying to get: " + URL ); Element n =
   * xnatfs.sFileHandleCache.get ( path ); if ( n != null ) { return
   * (FileHandle) n.getObjectValue (); } FileHandle fh = new FileHandle ( URL,
   * path ); n = new Element ( path, fh ); xnatfs.sFileHandleCache.put ( n );
   * return fh; }
   */
