/*
 * Copyright (c) 2002-2004, Martian Software, Inc. This file is made available
 * under the LGPL as described in the accompanying LICENSE.TXT file.
 */

/*
 * 09/26/2002 ML : Modified parseShortForm() and parseLongForm() to allow
 * options that begin with a minus sign. Previous behavior assumed that the
 * minus sign indicated another flag and therefore no value for the current
 * option.
 */
 ...
 * created and managed by a JSAP; developers should never need to access this
 * class directly.
 * 
...
class Parser {
  ...
  /**
   * Boolean indicating whether this Parser has already been run (a parser can
   * only run once).
   */
  private boolean parsed = false;

  /**
   * Boolean indicating whether this Parser has encountered the "--" argument.
   */
  private boolean foundDoubleDash = false;
   ...
   * 
   * @param config
   *          the JSAP for which this Parser is working.
   * @param args
   ...
   * (by declaration order). If there are no more expected UnflaggedOptions, the
   * current UnflaggedOption pointer is set to null.
   */
  ...
  /**
   * Instructs the specified parameter to parse the specified value, and stores
   * the results in the working JSAPResult. Any ParseExceptions or
   * IllegalMultipleDeclarationExceptions that are encountered are associated
   * with the specified parameter.
   * 
   * @param param
   *          the parameter to parse the value
   * @param valueToParse
   *          the value to parse
   */
  private void processParameter ( Parameter param, String valueToParse ) {
    ...
    if ( result.getObject ( param.getID () ) != null ) {
      if ( ( ( param instanceof FlaggedOption ) && ( !( (FlaggedOption) param ).allowMultipleDeclarations () ) ) || ( param instanceof Switch ) || ( param instanceof QualifiedSwitch )
          || ( ( param instanceof UnflaggedOption ) && ( !( (UnflaggedOption) param ).isGreedy () ) ) ) {
        result.addException ( param.getID (), new IllegalMultipleDeclarationException ( param.getID () ) );
      }
    }
  ...
  private int parseArg ( String[] args, int index ) {
    if ( foundDoubleDash ) {
      // Only parse unflagged options
      return ( parseUnflaggedOption ( args, index ) );
    }
    if ( args[index].equals ( "--" ) ) {
      foundDoubleDash = true;
      return parseArg ( args, index + 1 );
    }
   ...
   * values in the working JSAPResult
   */
      ...
      if ( result.getObject ( paramID ) == null ) {
        ...
        if ( paramValues != null ) {
          ...
          for ( int index = 0; index < z; ++index ) {
            processParameter ( config.getByID ( paramID ), paramValues[index] );
          }
        }
      }
    }
  ...
  /**
   * Checks the current JSAPResult against the JSAP's configuration, and stores
   * a RequiredParameterMissingException in the JSAPResult for any missing
   * parameters.
   */
      ...
      if ( o instanceof Option ) {
        ...
        if ( option.required () && ( result.getObject ( id ) == null ) ) {
          ...
          if ( result.getException ( id ) == null ) {
            result.addException ( option.getID (), new RequiredParameterMissingException ( id ) );
          }
        }
      }
    }
  ...
  /**
   * Loops through all parameters, informing the JSAPResult of any
   * QualifiedSwitches so it can assume they're not present.
   */
   ...
   * @param index
   *          the index of the current argument to parse
   * @return the index of the next argument to parse
   */
  private int parseLongForm ( String[] args, int index ) {
    ...
    // if no equals sign and no colon for QualifiedSwitches, this whole argument
    // is the long flag
    if ( equalsIndex == -1 && colonIndex == -1 ) {
      paramFlag = args[index].substring ( 2 );
    }
    if ( equalsIndex != -1 ) {
      // there's a value assignment in this argument, too; possible colons are
    ...
    if ( equalsIndex == -1 && colonIndex != -1 ) {
      ...
      paramColon = args[index].substring ( colonIndex ); // parameter starts
      // with ':'
    }
    // <<<<<<KPB
    ++index;
    ...
    for ( int charPos = 1; charPos < parseTo; ++charPos ) {
      ...
      if ( option == null ) {
        ...
        if ( option instanceof Switch ) {
           ...
           * 
           * if (charPos != (parseTo - 1)) { result.addException(param.getID(),
           * new SyntaxException("No value specified for option \"" + paramFlag
           * + "\".")); } else {
           */
  ...
  private int parseUnflaggedOption ( String[] args, int index ) {
    if ( curUnflaggedOption != null ) {
      ...
      result.addException ( null, new JSAPException ( "Unexpected argument: " + args[index] ) );
    }
    return ( index + 1 );
  }
